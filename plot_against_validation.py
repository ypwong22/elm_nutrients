...
folder = os.path.join('/lustre/haven/user/ywang254/E3SM/output/', 
                      prefix + '_plot' + plot + '_US-SPR_ICB20TRCNPRDCTCBC', 'run')
validation_file = '/lustre/haven/proj/UTK0134/Phenology_ELM/intermediate/' + \
...
def get_run_co2():
    ...
    data = pd.read_csv(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/', \
    ...
    return pd.Series(npp - rh.values, sim_tvec)
...
fig.savefig(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/',
                         prefix, 'plot' + plot + '_daily_nee.png'), dpi = 600.,
            bbox_inches = 'tight')
plt.close()
"""
...
def get_bnpp():
    ...
    npp = (data.loc[:,('hummock','NPP_pima' )] * 0.64 + \
          ...
          (data.loc[:,('hummock','NPP_lala' )] * 0.64 + \
           data.loc[:,('hollow' ,'NPP_lala' )] * 0.36) * 0.14 + \
          (data.loc[:,('hummock','NPP_shrub')] * 0.64 + \
           data.loc[:,('hollow' ,'NPP_shrub')] * 0.36) * 0.25
    anpp = (data.loc[:,('hummock','AGNPP_pima' )] * 0.64 + \
            data.loc[:,('hollow' ,'AGNPP_pima' )] * 0.36) * 0.36 + \
           (data.loc[:,('hummock','AGNPP_lala' )] * 0.64 + \
            data.loc[:,('hollow' ,'AGNPP_lala' )] * 0.36) * 0.14 + \
           (data.loc[:,('hummock','AGNPP_shrub')] * 0.64 + \
            data.loc[:,('hollow' ,'AGNPP_shrub')] * 0.36) * 0.25
    return (npp - anpp).groupby(npp.index.year).sum()
...
fig.savefig(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/' + 
                        prefix, 'plot' + plot + '_annual_npp.png'), dpi = 600.,
            bbox_inches = 'tight')
plt.close()
"""
...
fig.savefig(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/',
                         prefix, 'plot' + plot + '_daily_runoff.png'), dpi = 600.,
            bbox_inches = 'tight')
plt.close()
"""
...
fig.savefig(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/',
                         prefix, 'plot' + plot + '_daily_watertable.png'), dpi = 600.,
            bbox_inches = 'tight')
plt.close()
"""
...
RMSE.to_csv(os.path.join('/lustre/haven/proj/UTK0134/Phenology_ELM/output_elm/',
                         prefix, 'plot' + plot + '_RMSE.csv'))
