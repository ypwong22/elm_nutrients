...
def plotter_monthly(var_list, unit_list, minmax_list, filename):
    fig, axes = plt.subplots(nrows = len(var_list), ncols = 3, figsize = (14, 2.5 * len(var_list)), sharex = True, sharey = False)
    fig.subplots_adjust(hspace = 0.12, wspace = 0.05)

    for k, (varname, unit, minmax) in enumerate(zip(var_list, unit_list, minmax_list)):
        for i, (pft, pftname) in enumerate(zip([2, 3, 11], ['spruce', 'larch', 'shrub'])):
            ax = axes[k, i]
            for j, prefix in enumerate(sims_prefix):
                ...
                if '/' in varname:
                    ...
                    num = collection_ts.loc[:, (slice(None), num_var, pft, 'hummock')] * 0.64 + \
                        collection_ts.loc[:, (slice(None), num_var, pft, 'hollow')].values * 0.36
                    ...
                    denom = collection_ts.loc[:, (slice(None), denom_var, pft, 'hummock')] * 0.64 + \
                            collection_ts.loc[:, (slice(None), denom_var, pft, 'hollow')].values * 0.36
                    ...
                    frootc = collection_ts.loc[:, (slice(None), varname, pft, 'hummock')] * 0.64 + \
                            collection_ts.loc[:, (slice(None), varname, pft, 'hollow')].values * 0.36
                    ...
                    if unit == 'ratio to annual\naverage':
                        ...
                        frootc = frootc.groupby(frootc.index.year).apply(lambda df: df / df.mean(axis = 0))

                frootc = frootc.stack().to_frame()
                frootc_mean = frootc.groupby(frootc.index.get_level_values(0).month).mean().iloc[:, 0]
                frootc_std = frootc.groupby(frootc.index.get_level_values(0).month).std().iloc[:, 0]

                ax.plot(frootc_mean.index, frootc_mean, '-', color = clist[j], label = sims_names[j])
                ax.fill_between(frootc_mean.index, frootc_mean - frootc_std, frootc_mean + frootc_std, color = clist[j], alpha = 0.2)

                if varname in ['TLAI','FROOTC']:
                    if pft == 2:
                        if j == 0:
                            ax_inset = inset_axes(ax, width="80%", height="40%", loc="upper right")
                        ax_inset.plot(frootc_mean.index, frootc_mean, '-', color = clist[j], label = sims_names[j])
    ...
    fig.savefig(os.path.join(path_out, filename), dpi = 600., bbox_inches = 'tight')
    plt.close(fig)


def plotter_annual(var_list, unit_list, minmax_list, filename):
    fig, axes = plt.subplots(nrows = len(var_list), ncols = 3, figsize = (14, 2.2 * len(var_list)), sharex = True, sharey = False)
    fig.subplots_adjust(hspace = 0.12, wspace = 0.05)
    for k, (varname, unit, minmax) in enumerate(zip(var_list, unit_list, minmax_list)):
        for i, (pft, pftname) in enumerate(zip([2, 3, 11], ['spruce', 'larch', 'shrub'])):
            ax = axes[k, i]
