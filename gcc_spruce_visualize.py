...
def parse_sapflow():
    ...
    for plots in np.unique(raw['plot']):
        ...
        temp = raw.loc[(raw['plot'] == plots) & (raw['Species'] == 'Pima'), :]
...
def find_rcc_dates(rcc_series, file, sitename, overwrite = False):
    ...
    if os.path.exists(ta_file) and not overwrite:
        ...
        ta = get_daymetT(file, sitename).loc[rcc_series.index]
        ta.to_csv(ta_file)
...
snow_ground = pd.read_csv(os.path.join(path_input, '2015_20200331_Spruce_Snow_On_Ground_Flags.csv'),
                          index_col = 0, parse_dates = True)
snow_trees  = pd.read_csv(os.path.join(path_input, '2015_20200331_Spruce_Snow_On_Trees_Flags.csv'),
                          index_col = 0, parse_dates = True)
sapflow_dates = parse_sapflow()

####################################################################################################
# Time series
####################################################################################################
for t,c in it.product(tlevels, aco2 + eco2):
    flist = glob(os.path.join(path_in, 'data_record_4', 'spruceT%dP%02d*_EN_*_3day.csv' % (t,c)))
    for f in flist:
        data = pd.read_csv(f, skiprows = 24, index_col = 0, parse_dates = True)

        sitename  = f.split('_')[-4].split('spruce')[-1]
        roi       = f.split('_')[-2]
        gcc_dates = parse_gcc_dates(sitename, roi)
        rcc_dates = find_rcc_dates (data['smooth_rcc_90'], f, sitename, False)
        ...
        for i, th, mk in zip(range(3),
                              ['transition_10', 'transition_25', 'transition_50'],
                              ['o', '^', 'v']):
            h10[i], = ax.plot(gcc_dates[('rising', th)], [0.36] * len(gcc_dates[('rising', th)]),
                              mk, markersize = 2, color = 'g')
            ax.plot(gcc_dates[('falling', th)], [0.36] * len(gcc_dates[('falling', th)]), 
                    mk, markersize = 2, color = 'g')
        ...
        fig.savefig(os.path.join(path_out, '%s_%s.png' % (sitename,roi)), 
...
for t, (c,co2) in it.product(tlevels, zip(aco2 + eco2, ['a']*len(aco2) + ['e']*len(eco2))):
    ...
    for f in flist:
        ...
        roi       = f.split('_')[-2]
        gcc_dates = parse_gcc_dates(sitename, roi)
        rcc_dates = find_rcc_dates (data['smooth_rcc_90'], f, sitename)
        ...
        for y,dir in it.product(range(2015, 2020), ['rising', 'falling']):
            ...
            if len(n) > 0:
                dayofyear.loc[count, 'sapflow'] = sapflow_dates[c][dir][n[0]].strftime('%Y-%m-%d')
            for s in [10,25,50]:
                n = np.where(pd.Index(gcc_dates[(dir, 'transition_%s' % s)]).year == y)[0]
                if len(n) > 0:
                    dayofyear.loc[count, 'gcc_90_transition_%s' % s] = \
                        gcc_dates[(dir, 'transition_%s' % s)][n[0]].strftime('%Y-%m-%d')
            ...
            count += 1
dayofyear.to_csv(os.path.join(path_out, 'summary_dayofyear.csv'), index = False)

dayofyear = pd.read_csv(os.path.join(path_out, 'summary_dayofyear.csv'))
...
ax.legend([h1] + h2 + [h3], ['sapflow','gcc_90_transition_10', 'gcc_90_transition_25', 
                             'gcc_90_transition_50', 'rcc_90_transition'],
          ncol = 3, loc = (-1.2, -0.4))
fig.savefig(os.path.join(path_out, 'summary_dayofyear.png'), dpi = 600., bbox_inches = 'tight')
plt.close(fig)
...
for t,c in it.product(tlevels, aco2 + eco2):
    ...
    for f in flist:
        ...
        for j, th, mk in zip(range(3),
                              ...
                              ['o', '^', 'v']):
            adjusted = []
            for i in gcc_dates[('rising', th)] + gcc_dates[('falling', th)]:
                if (i.month == 2) & (i.day == 29):
                    adjusted.append(datetime(year = 1999, month = 2, day = 28))
                else:
                    adjusted.append(datetime(year = 1999, month = i.month, day = i.day))
        ...
        fig.savefig(os.path.join(path_out, 'cycle_%s_%s.png' % (sitename,roi)), 
                    dpi = 600, bbox_inches = 'tight')
        plt.close(fig)
