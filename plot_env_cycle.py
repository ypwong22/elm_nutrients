...
obs_tsoi_5 = (hr['halfhourly_tsoil_hummock'][:, :, 1] * 0.64 + \
              hr['halfhourly_tsoil_hollow'][:, :, 1] * 0.36).resample({'time': '1MS'}).mean()
obs_tsoi_5 = pd.DataFrame(obs_tsoi_5.values, index = obs_tsoi_5['time'].to_index(), columns = hr['chamber'])
# 10 cm
obs_tsoi_10 = (hr['halfhourly_tsoil_hummock'][:, :, 2] * 0.64 + \
               hr['halfhourly_tsoil_hollow'][:, :, 2] * 0.36).resample({'time': '1MS'}).mean()
obs_tsoi_10 = pd.DataFrame(obs_tsoi_10.values, index = obs_tsoi_10['time'].to_index(), columns = hr['chamber'])
# hummock & hollo
obs_swc = hr['halfhourly_swc_hummock'].resample({'time': '1MS'}).mean() * 0.64 + \
          hr['halfhourly_swc_hollow'].resample({'time': '1MS'}).mean() * 0.36
obs_swc = pd.DataFrame(obs_swc.values, index = obs_swc['time'].to_index(), columns = hr['chamber'])
hr.close()
...
fig, axes = plt.subplots(nrows = len(var_list), ncols = 3, figsize = (12, 2 * len(var_list)), sharex = False, sharey = False)
...
for k, (varname, unit) in enumerate(zip(var_list, unit_list)):
    for j, prefix in enumerate(sims_prefix):
        ...
        month = pd.DatetimeIndex([datetime(m, n, 1) for m,n in zip(collection_ts.index.get_level_values(0).year, collection_ts.index.get_level_values(0).month)])

        if varname in ['SWC', 'H2OSOI']:
            var = (collection_ts.loc[:, (slice(None), f'{varname}_1', 0, 'hummock')] * 0.64 + \
                   collection_ts.loc[:, (slice(None), f'{varname}_1', 0, 'hollow')].values * 0.36) * 0.5 + \
                  (collection_ts.loc[:, (slice(None), f'{varname}_5', 0, 'hummock')] * 0.64 + \
                   collection_ts.loc[:, (slice(None), f'{varname}_5', 0, 'hollow')].values * 0.36).values * 0.5 
        else:
            var = collection_ts.loc[:, (slice(None), varname, 0, 'hummock')] * 0.64 + \
                  collection_ts.loc[:, (slice(None), varname, 0, 'hollow')].values * 0.36

        var_chamber = var.mean(axis = 0)
        t_chamber = collection_ts.loc[:, (slice(None), 'TBOT', 0, 'hummock')] * 0.64 + \
        ...
        var_annual = var.groupby(var.index.get_level_values(0).year).mean().values.reshape(-1)
        var_seasonal = var.groupby(var.index.get_level_values(0).month).mean().values.reshape(-1)
        var_seasonal_std = var.groupby(var.index.get_level_values(0).month).std().values.reshape(-1)

        ax = axes[k, 0]
        ax.plot(range(1,13), var_seasonal, '-', color = clist[j], label = sims_names[j])
        ax.fill_between(range(1,13), var_seasonal - var_seasonal_std, var_seasonal + var_seasonal_std, color = clist[j], alpha = 0.2)
        if varname.startswith('SWC_') or varname.startswith('H2OSOI_') or varname.startswith('TSOI_'):
    ...
    if varname in ['TSOI_3', 'TSOI_4', 'SWC', 'H2OSOI']:
        ...
        ax.fill_between(obs_seasonal.index, obs_seasonal - obs_seasonal_std, obs_seasonal + obs_seasonal_std, color = 'k', alpha = 0.2)
        ax = axes[k, 1]
...
fig.savefig(os.path.join(path_out, f'plot_env_cycle.png'), dpi = 600., bbox_inches = 'tight')
plt.close(fig)