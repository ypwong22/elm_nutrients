...
path_data = os.path.join(os.environ['SCRATCHDIR'], 'E3SM', 'output', 
                         '20211008_' + level + '_US-SPR_ICB20TRCNPRDCTCBC','run')
path_out = os.path.join(os.environ['PROJDIR'], 'Phenology_ELM')
...
for yy in range(2016,2019):
    flist = glob(os.path.join(path_data, '*h1*' + str(yy - 165) + '*.nc'))
    hr = xr.open_mfdataset(flist, decode_times = True)
...
fig.savefig(os.path.join(path_out, level + '_2.png'), dpi = 600., bbox_inches = 'tight')
plt.close()
...
fig.savefig(os.path.join(path_out, level + '_3.png'), dpi = 600., bbox_inches = 'tight')
plt.close()
