...
if newfile.endswith("_NP"):
    ...
    hr["VMAX_PLANT_N_RD"] = xr.DataArray(
        [
            np.nan, np.nan, 1e-7, 1e-7, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, 1e-7, 1e-7, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan,
        ],
        coords={"pft": hr["pft"]},
        dims=["pft"],
        attrs={
            "units": "gN gC-1 s-1", "long_name": "Paramter for plant uptake of nitrogen proportional to fine root biomass",
        },
    )
