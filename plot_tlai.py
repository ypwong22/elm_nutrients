...
for i, (pft, pftname, pftname2) in enumerate(
    zip(pft_list, pft_names, ["EN", "DN", "SH"])
):
    ...
    for plot in chamber_list:
        obs = pd.Series(
            hr["annual_lai"].loc[:, plot, pftname2].values, index=hr["year"]
        )
    ...
    for j, expr in enumerate(sims_prefix):
        ...
        for plot, plot_name in zip(chamber_list, chamber_list_names):
            ...
            sim = (
                collection_ts.loc[:, (plot, "TLAI", pft, "hummock")] * 0.64
                + collection_ts.loc[:, (plot, "TLAI", pft, "hollow")] * 0.36
            )
            sim = sim.groupby(
                pd.DatetimeIndex(
                    [datetime(y, m, 1) for y, m in zip(sim.index.year, sim.index.month)]
                )
            ).mean()
...
ax.legend(
    h,
    ["OBS_ACO2", "MOD ACO2", "OBS ECO2", "MOD ECO2"],
    ncol=4,
    columnspacing=1,
    loc=[-1, -0.5],
)
fig.savefig(os.path.join(path_out, f"plot_tlai.png"), dpi=600.0, bbox_inches="tight")
plt.close(fig)
hr.close()
